...
//    rv = gpio_request(IRQ_QSC_INT, "IRQ_EINT14");
 //   if(rv < 0) {
 //       printk("IDPRAM: [%s] failed to get IRQ_EINT14\n",__func__);
 //       goto err;
 //   }
...
//    s3c_gpio_cfgpin(GPIO_C210_DPRAM_INT_N, S3C_GPIO_SFN(0xFF));
 ...
 //   rv = gpio_request(IRQ_QSC_ACTIVE, "QSC_ACTIVE");
 //   if(rv < 0) {
 //       printk("IDPRAM: [%s] failed to get QSC_ACTIVE\n",__func__);
 //       goto err1;
  //  }
    phone_active_irq = gpio_to_irq(IRQ_QSC_ACTIVE);
...
//         printk("IDPRAM: [%s] failed to get gpio GPY4_2\n",__func__);
 ...
 //       }
  ...
  //      rv = gpio_request(GPIO_QSC_RESET_N, "GPX1_4");
  //      if(rv < 0) {
 //               printk("IDPRAM: [%s] failed to get gpio GPX1_4\n",__func__);
 //               goto err4;
 //       }

 //       s5p_gpio_set_drvstr(GPIO_QSC_RESET_N, S5P_GPIO_DRVSTR_LV4); //To increase driving strength of this pin
...
static int register_interrupt_handler(void)
{
    ...
    retval = request_irq(dpram_wakeup_irq, idpram_wake_from_CP_irq_handler, IRQF_DISABLED, "QSC_INT irq", NULL);
	if (retval) {
		LOGE("DPRAM interrupt handler failed.\n");
