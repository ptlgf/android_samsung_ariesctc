...
//truct completion g_complet_dpramdown;
...
static int dpram_read_raw(dpram_device_t *device, const u16 non_cmd)
{
	...
	if(head != tail) {
		...
		while(size){			
			...
			if(!ret) {
				LOGE("(tty_insert_flip_string) drop byte: %d\n buff addr: %x\n read addr: %x\n", 
...
{
	send_interrupt_to_phone_with_semaphore(INT_COMMAND(MBX_CMD_RES_ACTIVE));
}
...
static void phone_active_delayed_work_handler(struct work_struct *ignored)
{
	...
	if(gpio_get_value(GPIO_QSC_ACTIVE) != 0) {
...
//	set_irq_type(IRQ_QSC_ACTIVE,IRQ_TYPE_EDGE_BOTH);
	...
	if (gpio_is_valid(GPIO_QSC_PHONE_ON)) {
		if (gpio_request(GPIO_QSC_PHONE_ON, "GPIO_QSC_PHONE_ON"))
	...
	if (gpio_is_valid(GPIO_QSC_RESET_N)) {
		if (gpio_request(GPIO_QSC_RESET_N, "GPIO_QSC_RESET_N"))
